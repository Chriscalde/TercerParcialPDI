...
class CursoController extends ValidarController
{
    ...
    public function create(Request $request)
    {
        ...
        $result = DB::select('CALL crearNuevoCurso(?,?,?,?)', array($name,$init,$end,$valid));
    ...
    public function update(Request $request)
    {
        ...
        $end = $this->validateDatetime($request->input('end'));
        $valid = $this->valueToBool($request->input('active'));
        $curso = DB::select('CALL editarCurso(?,?,?,?,?)', array($id, $name, $init, $end, $valid));
        return redirect()->action('CursoController@list');
    }
    ...
    {
        $id = $request->input('id');
        $curso = DB::select('CALL eliminarCurso(?)', array($id));
        return redirect()->action('CursoController@list');
    }
}
